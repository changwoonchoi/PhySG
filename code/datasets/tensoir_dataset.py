...
class TensoIRDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        json_path = os.path.join(self.instance_dir, f'transforms_{split}.json')
        ...
        img_h, img_w = rend_util.load_rgb(image_paths[0]).shape[1:]
        ...
        for i in range(self.n_cameras):
            ...
            self.pose_all.append(torch.from_numpy(poses[i]))
        ...
        if len(image_paths) > 0:
            ...
            H, W = rend_util.load_rgb(image_paths[0]).shape[1:]
            self.img_res = [H, W]
